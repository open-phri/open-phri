...
int main(int argc, char const *argv[]) {
	...
	std::function<void(void)> runner = [&safety_controller](){safety_controller.compute();};
	...
	safety_controller.add("ec cstr", KineticEnergyConstraint(emax, mass_eq.getEquivalentMass()));
	runner = [&safety_controller, &mass_eq](){mass_eq.compute(); safety_controller.compute();};
	run_benchmark();



	// Velocity + power + kinetic energy constraint + PFM + Stiffness + force control
	logger.reset();
