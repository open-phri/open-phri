...
class DataReplayer {
public:
	DataReplayer(const std::string& file_path, ArrayT& array, size_t size, SkipRows skip_rows = SkipRows(0), SkipCols skip_cols = SkipCols(0)) :
		array_(array),
		size_(size),
...
private:
	void init(const std::string& file_path) {
		...
		if (log_file_.is_open()) {
			...
			throw std::runtime_error(OPEN_PHRI_ERROR("Cannot open the file " + file_path + " for reading"));
		}
	}
	...
	template<typename T>
	using element_type_t = std::remove_reference_t<decltype(std::declval<T>()[0])>;

	template<typename T = ArrayT>
	void set(size_t index, element_type_t<typename T::element_type> value, typename std::enable_if<is_shared_ptr<T>::value>::type* = nullptr) {
		(*array_)[index] = value;
	}

	template<typename T = ArrayT>
	void set(size_t index, element_type_t<T> value, typename std::enable_if<not is_shared_ptr<T>::value>::type* = nullptr) {
		array_[index] = value;
	}
