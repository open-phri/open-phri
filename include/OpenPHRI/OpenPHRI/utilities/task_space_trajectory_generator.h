...
public:
	TrajectoryGenerator(const TrajectoryPoint<Pose>& start, double sample_time, TrajectorySynchronization sync = TrajectorySynchronization::NoSynchronization, TrajectoryOutputType output_type = TrajectoryOutputType::All) :
		...
		start_pose_(start)
	{
		...
		task_space_acceleration_output_ = std::make_shared<Acceleration>(*start.d2y);
	}

	void addPathTo(const TrajectoryPoint<Pose>& to, const Twist& max_velocity, const Acceleration& max_acceleration) {
	...
	virtual bool computeTimings(double v_eps = 1e-6, double a_eps = 1e-6) override {
		updatePoints();
	...
	virtual bool compute() override {
		...
		if(output_type_ == TrajectoryOutputType::Position or output_type_ == TrajectoryOutputType::All) {
			pose_output_->translation() = position_output_->block<3,1>(0,0);
			pose_output_->orientation() = start_pose_.y->orientation().integrate(position_output_->block<3,1>(3,0));
		}
		if(output_type_ == TrajectoryOutputType::Velocity or output_type_ == TrajectoryOutputType::All) {
			*twist_output_ = *velocity_output_;
		}
		if(output_type_ == TrajectoryOutputType::Acceleration or output_type_ == TrajectoryOutputType::All) {
