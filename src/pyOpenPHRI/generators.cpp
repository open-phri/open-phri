...
std::shared_ptr<PotentialFieldObject> NewPotentialFieldObject(PotentialFieldType type, doubleConstPtr gain, doubleConstPtr threshold_distance, Vector6dConstPtr object_position)
{
	return std::make_shared<PotentialFieldObject>(type, gain, threshold_distance, object_position);
}

std::shared_ptr<PotentialFieldGenerator> NewPotentialFieldGenerator(Vector6dConstPtr robot_position)
{
...
std::shared_ptr<StiffnessGenerator> NewStiffnessGenerator(Matrix6dConstPtr stiffness, Vector6dConstPtr target_position, Vector6dConstPtr robot_position)
{
	return std::make_shared<StiffnessGenerator>(stiffness, target_position, robot_position);
}
BOOST_PYTHON_FUNCTION_OVERLOADS(NewStiffnessGenerator_overloads, NewStiffnessGenerator, 2, 3)

std::shared_ptr<MassGenerator> NewMassGenerator(Matrix6dConstPtr mass, Vector6dConstPtr target_acceleration)
{
	return std::make_shared<MassGenerator>(mass, target_acceleration);
}

std::shared_ptr<MassGenerator> NewMassGenerator(Matrix6dConstPtr mass, Vector6dConstPtr target_acceleration, Vector6dConstPtr robot_acceleration)
{
	return std::make_shared<MassGenerator>(mass, target_acceleration, robot_acceleration);
}
...
void wrapGenerators() {
	...
	def("NewPotentialFieldGenerator",   (std::shared_ptr<PotentialFieldGenerator>(*)(Vector6dPtr)) 0,
	    NewPotentialFieldGenerator_overloads(
			args("rob_pos"),                                            "Create a new instance of a PotentialFieldGenerator shared_ptr"));

	def("NewStiffnessGenerator",        (std::shared_ptr<StiffnessGenerator>(*)(Matrix6dConstPtr, Vector6dConstPtr, Vector6dConstPtr)) 0,
	    NewStiffnessGenerator_overloads(
			args("stiffness", "target_position", "robot_position"),     "Create a new instance of a StiffnessGenerator shared_ptr"));

	def("NewMassGenerator",             (std::shared_ptr<MassGenerator>(*)(Matrix6dConstPtr, Vector6dConstPtr, Vector6dConstPtr)) 0,
	    NewMassGenerator_overloads(
			args("mass", "target_acceleration", "robot_acceleration"),  "Create a new instance of a MassGenerator shared_ptr"));
	...
	class_<VelocityProxy, boost::noncopyable, bases<VelocityGenerator>>("VelocityProxy", no_init)
	...
	class_<ForceProxy, boost::noncopyable, bases<ForceGenerator>>("ForceProxy", no_init)
	...
	class_<ObjectCollection<PotentialFieldObjectPtr>>("PotentialFieldObjectCollection", no_init);

	void (PotentialFieldGenerator:: *PotentialFieldGenerator_setVerbose)(bool) = &PotentialFieldGenerator::setVerbose;
	...
	class_<MassGenerator, boost::noncopyable, bases<ForceGenerator>>("MassGenerator", no_init)
	...
	implicitly_convertible<std::shared_ptr<StiffnessGenerator>,         std::shared_ptr<ForceGenerator>>();
	implicitly_convertible<std::shared_ptr<MassGenerator>,              std::shared_ptr<ForceGenerator>>();
}
