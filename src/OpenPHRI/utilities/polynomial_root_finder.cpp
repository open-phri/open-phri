/*      File: polynomial_root_finder.cpp
...
void Fxshfr_ak1(int L2, int* NZ, double sr, double v, double K[MDP1], int N,
                double p[MDP1], int NN, double qp[MDP1], double u, double* lzi,
...
void QuadIT_ak1(int N, int* NZ, double uu, double vv, double* szr, double* szi,
                ...
                double* a3, double* a7, double* c, double* d, double* e,
                double* f, double* g, double* h, double K[MDP1]);
...
void rpoly_ak1(double op[MDP1], int* Degree, double zeror[MAXDEGREE],
    ...
    const double lb2 = std::log(
        2.0); // Dummy variable to avoid re-calculating this value in loop below
    const double lo = DBL_MIN / DBL_EPSILON;
    const double cosr = std::cos(94.0 * RADFAC); // = -0.069756474
    const double sinr = std::sin(94.0 * RADFAC); // = 0.99756405

    if ((*Degree) > MAXDEGREE) {
        std::cout << "\nThe entered Degree is greater than MAXDEGREE. Exiting "
                     "rpoly. No further action taken.\n";
        *Degree = -1;
        return;
    ...
    if (op[0] != 0) {
        ...
        while (N >= 1) { // Main loop
            ...
            x = exp((std::log(-pt[N]) - std::log(pt[0])) / (double)N);
            ...
            for (jj = 0; jj < 5; jj++) {
                ...
                if (zerok) {
                    ...
                    zerok = ((K[NM1] == 0) ? 1 : 0);
                       ...
                       // Used scaled form of recurrence if value of K at 0 is
                       // nonzero
                    t = -aa / cc;
                    for (i = 0; i < NM1; i++) {
                        j = NM1 - i;
                    ...
                    K[0] = p[0];
                    zerok =
                        ((std::abs(K[NM1]) <= std::abs(bb) * DBL_EPSILON * 10.0)
                             ? 1
                             : 0);
                } // End else !zerok
            ...
            for (jj = 1; jj <= 20; jj++) {
                ...
                // and its complex conjugate. The point has modulus BND and
                ...
                Fxshfr_ak1(20 * jj, &NZ, sr, bnd, K, N, p, NN, qp, u, &lzi,
                           &lzr, &szi, &szr);
                ...
                if (NZ != 0) {
                    ...
                    // polynomial, store the zero or zeros, and return to the
                    // main algorithm.
            ...
            if (jj > 20) {
                std::cout << "\nFailure. No convergence after 20 shifts. "
                             "Program terminated.\n";
                *Degree -= N;
                break;
            } // End if (jj > 20)
        ...
        std::cout << "\nThe leading coefficient is zero. No further action "
                     "taken. Program terminated.\n";
        *Degree = 0;
    } // End else op[0] == 0
    ...
    *NZ = 0;
    ...
    for (j = 0; j < L2; j++) {
        ...
        tFlag = calcSC_ak1(N, a, b, &a1, &a3, &a7, &c, &d, &e, &f, &g, &h, K, u,
                           v, qk);
        newest_ak1(tFlag, &ui, &vi, a, a1, a3, a7, b, c, d, f, g, h, u, v, K, N,
                   p);
            ...
            if ((spass) || (vpass)) {
                ...
                for (;;) {
                    ...
                    if ((fflag && ((fflag = 0) == 0)) &&
                        ...
                        QuadIT_ak1(N, NZ, ui, vi, szr, szi, lzr, lzi, qp, NN,
                                   &a, &b, p, qk, &a1, &a3, &a7, &c, &d, &e, &f,
                                   &g, &h, K);
                        ...
                        // Try linear iteration if it has not been tried and the
                        // s sequence is converging
                        if (stry || (!spass)) {
                            iFlag = 0;
                        } // End if (stry || (!spass))
                        else {
                    ...
                    if (iFlag != 0) {
                        RealIT_ak1(&iFlag, NZ, &s, N, p, NN, qp, szr, szi, K,
                                   qk);
                    ...
                    // Try quadratic iteration if it has not been tried and the
                    // v sequence is converging
                ...
                QuadSD_ak1(NN, u, v, p, qp, &a, &b);
                tFlag = calcSC_ak1(N, a, b, &a1, &a3, &a7, &c, &d, &e, &f, &g,
                                   &h, K, u, v, qk);
...
void QuadIT_ak1(int N, int* NZ, double uu, double vv, double* szr, double* szi,
    ...
    *NZ = 0; // Number of zeros found
    ...
    do {
        ...
        ee = (9.0 * ee + 2.0 * std::abs(t) -
              7.0 * (std::abs((*a) + t) + zm * std::abs((*b)))) *
             DBL_EPSILON;
        ...
        if (j >= 2) {
            if ((relstp <= 0.01) && (mp >= omp) && (!triedFlag)) {
                ...
                relstp = ((relstp < DBL_EPSILON) ? std::sqrt(DBL_EPSILON)
                                                 : std::sqrt(relstp));
                ...
                for (i = 0; i < 5; i++) {
                    tFlag = calcSC_ak1(N, *a, *b, a1, a3, a7, c, d, e, f, g, h,
                                       K, u, v, qk);
                    nextK_ak1(N, tFlag, *a, *b, *a1, a3, a7, K, qk, qp);
                } // End for i
        ...
        tFlag =
            calcSC_ak1(N, *a, *b, a1, a3, a7, c, d, e, f, g, h, K, u, v, qk);
        newest_ak1(tFlag, &ui, &vi, *a, *a1, *a3, *a7, *b, *c, *d, *f, *g, *h,
                   u, v, K, N, p);
    ...
    for (;;) {
        ...
        if (j >= 2) {
            if ((std::abs(t) <= 0.001 * std::abs(-t + s)) && (mp > omp)) {
                ...
                *sss = s;
                break;
            } // End if ((std::abs(t) <= 0.001*std::abs(s - t)) && (mp > omp))
        ...
        if (std::abs(kv) > std::abs(K[nm1]) * 10.0 * DBL_EPSILON) {
            ...
            for (i = 1; i < N; i++)
                K[i] = t * qk[i - 1] + qp[i];
        }      // End if (std::abs(kv) > std::abs(K[nm1])*10.0*DBL_EPSILON)
        else { // else (std::abs(kv) <= std::abs(K[nm1])*10.0*DBL_EPSILON)
               // Use unscaled form
            K[0] = 0.0;
            for (i = 1; i < N; i++)
                K[i] = qk[i - 1];
        } // End else (std::abs(kv) <= std::abs(K[nm1])*10.0*DBL_EPSILON)

        kv = K[0];
        for (i = 1; i < N; i++)
            kv = kv * s + K[i];

        t = ((std::abs(kv) > (std::abs(K[nm1]) * 10.0 * DBL_EPSILON))
                 ? -(pv / kv)
                 : 0.0);
...
void Quad_ak1(double a, double b1, double c, double* sr, double* si, double* lr,
    ...
    // larger zero if the zeros are real and both zeros are complex. The smaller
    // real zero is found directly from the product of the zeros c/a.
    ...
    *sr = *si = *lr = *li = 0.0;
    ...
    }      // End if (std::abs(b) < std::abs(c))
    else { // Else (std::abs(b) >= std::abs(c))
        e = -((a / b) * (c / b)) + 1.0;
        d = std::sqrt(std::abs(e)) * (std::abs(b));
    } // End else (std::abs(b) >= std::abs(c))
