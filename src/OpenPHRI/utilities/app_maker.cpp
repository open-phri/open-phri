...
{
	std::cout << "[OpenPHRI::AppMaker] Parsing the configuration file..." << std::flush;
	auto conf = YAML::LoadFile(PID_PATH(configuration_file));
	std::cout << " done." << std::endl;

	/***				Robot				***/
	std::cout << "[OpenPHRI::AppMaker] Creating a new robot..." << std::flush;
	impl_->robot = std::make_shared<Robot>();
	std::cout << " done." << std::endl;

	std::cout << "[OpenPHRI::AppMaker] Loading the robot model..." << std::flush;
	impl_->model = std::make_shared<RobotModel>(
		impl_->robot,
		conf);
	std::cout << " done." << std::endl;

	std::cout << "[OpenPHRI::AppMaker] Configuring the robot with the model parameters..." << std::flush;
	impl_->robot->create(impl_->model->name(), impl_->model->jointCount());
	std::cout << " done." << std::endl;

	/***				Robot driver				***/
	std::cout << "[OpenPHRI::AppMaker] Creating the robot driver..." << std::flush;
	...
	std::cout << "[OpenPHRI::AppMaker] Starting the robot driver..." << std::flush;
	impl_->driver->start();
	std::cout << " done." << std::endl;

	/***			Controller configuration			***/
	std::cout << "[OpenPHRI::AppMaker] Creating the robot controller..." << std::flush;
	impl_->controller = std::make_shared<SafetyController>(impl_->robot, conf);
	std::cout << " done." << std::endl;

	/***			Data logger configuration			***/
	std::cout << "[OpenPHRI::AppMaker] Creating the data logger..." << std::flush;
...
bool AppMaker::init(std::function<bool(void)> init_code) {
	bool all_ok = true;
	std::cout << "[OpenPHRI::AppMaker] Initializing the robot..." << std::flush;
	all_ok &= impl_->driver->init(impl_->init_timeout);
	std::cout << " done." << std::endl;
	if(init_code) {
		std::cout << "[OpenPHRI::AppMaker] Calling user initialization function..." << std::flush;
		all_ok &= init_code();
		std::cout << " done." << std::endl;
	}
	return all_ok;
...
bool AppMaker::stop() {
	std::cout << "[OpenPHRI::AppMaker] Stopping the robot..." << std::flush;
