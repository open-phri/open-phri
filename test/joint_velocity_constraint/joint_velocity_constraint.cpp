...
int main(int argc, char const *argv[]) {
	...
	*maximum_velocities << 1., 2., 3.;
	auto velocity_constraint = make_shared<JointVelocityConstraint>(maximum_velocities);

	auto constant_vel = make_shared<VectorXd>(3);
	*constant_vel << 0., 0., 0.;
	auto constant_velocity_generator = make_shared<JointVelocityProxy>(constant_vel);
	...
	assert_msg("Step #2", robot->jointVelocity()->isApprox(*robot->jointTotalVelocity()));

	// Step #3 : velocity 1 axis > max
	(*constant_vel)(0) = 1.5;
	safety_controller.compute();

	assert_msg("Step #3", isLessOrEqual(*robot->jointVelocity(), *maximum_velocities));
	...
	assert_msg("Step #5", isLessOrEqual(*robot->jointVelocity(), *maximum_velocities));
