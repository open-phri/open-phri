...
int main(int argc, char const *argv[]) {
	...
	*robot->controlPointDampingMatrix() *= 10.;
	...
	assert_msg("Step #1", isClose(power(robot->controlPointVelocity(), external_force), 0.));

	// Step #2 : velocity 1 axis, no force
	constant_vel->translation().x() = 0.2;
	safety_controller.compute();

	assert_msg("Step #2", isClose(power(robot->controlPointVelocity(), external_force), 0.));

	// Step #3 : velocity 1 axis, force same axis with opposite sign < max
	(*external_force)(0) = -10.;
	safety_controller.compute();

	assert_msg("Step #3", isClose(power(robot->controlPointVelocity(), external_force), -2.));

	// Step #4 : velocity 1 axis, force same axis with same sign < max
	(*external_force)(0) = 10.;
	safety_controller.compute();

	assert_msg("Step #4", isClose(power(robot->controlPointVelocity(), external_force), 2.));

	// Step #5 : velocity 1 axis, force same axis with opposite sign > max
	(*external_force)(0) = -100.;
	safety_controller.compute();

	assert_msg("Step #5", isClose(power(robot->controlPointVelocity(), external_force), -*maximum_power));

	// Step #6 : velocity 1 axis, force same axis with same sign > max
	(*external_force)(0) = 100.;
	safety_controller.compute();

	assert_msg("Step #6", static_cast<Vector6d>(*robot->controlPointVelocity()).isApprox(static_cast<Vector6d>(*robot->controlPointTotalVelocity())));
